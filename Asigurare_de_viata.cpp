///Acest program e facut sa introduca datele asiguratului si sa calculeze prima lunara
...
public:
    ...
    beneficiar& operator=(const beneficiar& nou) {
        if (this == &nou) return *this;

        delete[] numeBeneficiar;
        numeBeneficiar = new char[strlen(nou.numeBeneficiar) + 1];
        strcpy(numeBeneficiar, nou.numeBeneficiar);
        varstaBeneficiar = nou.varstaBeneficiar;
        procentOferit = nou.procentOferit;
   ...
   char* getNumeBeneficiar () const
{
    ...
    char[strlen(numeBeneficiar) + 1];
    strcpy(numeBeneficiarReturnat , numeBeneficiar);
...
friend std::ostream& operator<<(std::ostream& out, const beneficiar& b) {
    out << "Beneficiar: " << b.numeBeneficiar << std::endl;
    out << "Varsta: " << b.varstaBeneficiar << std::endl;
    out << "Procent: " << b.procentOferit << "%" << std::endl;
    return out;
}
...
Job& operator=(const Job& nou) {
    ...
    locDeMunca = new char[strlen(nou.locDeMunca) + 1];
    strcpy(locDeMunca, nou.locDeMunca);

    firma = new char[strlen(nou.firma) + 1];
    strcpy(firma, nou.firma);

    rolMunca = new char[strlen(nou.rolMunca) + 1];
    strcpy(rolMunca, nou.rolMunca);

    salariuDoi  = nou.salariuDoi;
    nivelRisc   = nou.nivelRisc;
    prima       = nou.prima;
    sumaAsigurata = nou.sumaAsigurata;
...
char* getLocDeMunca () const
{
    ...
    char[strlen(locDeMunca) + 1];
    strcpy(locDeMuncaReturnat , locDeMunca);
...
FisaMedicala& operator=(const FisaMedicala& nou) {
    if (this == &nou) return *this;
    ...
    numeMedic = new char[strlen(nou.numeMedic) + 1];
    strcpy(numeMedic, nou.numeMedic);

    dataConsultatiei = new char[strlen(nou.dataConsultatiei) + 1];
    strcpy(dataConsultatiei, nou.dataConsultatiei);

    istoricBoli = new char[strlen(nou.istoricBoli) + 1];
    strcpy(istoricBoli, nou.istoricBoli);

    varstaDoi       = nou.varstaDoi;
    greutate        = nou.greutate;
    inaltime        = nou.inaltime;
    fumat           = nou.fumat;
    alcool          = nou.alcool;
    activitateFizica = nou.activitateFizica;
    aprobat         = nou.aprobat;
...
Cumparatori& operator=(const Cumparatori& nou) {
    if (this == &nou) return *this;
    ...
    delete[] adresa;
    delete[] emisDe;
    delete[] adresaDeEmail;


    nume = new char[strlen(nou.nume) + 1];
    strcpy(nume, nou.nume);

    judet = new char[strlen(nou.judet) + 1];
    strcpy(judet, nou.judet);

    serieBuletin = new char[strlen(nou.serieBuletin) + 1];
    strcpy(serieBuletin, nou.serieBuletin);

    adresa = new char[strlen(nou.adresa) + 1];
    strcpy(adresa, nou.adresa);

    emisDe = new char[strlen(nou.emisDe) + 1];
    strcpy(emisDe, nou.emisDe);

    adresaDeEmail = new char[strlen(nou.adresaDeEmail) + 1];
    strcpy(adresaDeEmail, nou.adresaDeEmail);
...
Cumparatori demoClient("Ion Popescu", "Bucuresti",
                       "AB123456", "Str. Florilor 1",
                       "SPCLEP", "ion@gmail.com");

FisaMedicala demoFisa("Dr. Ionescu", "2024-01-15",
                      "Fara boli cronice", 35,
                      75.0, 1.75f, false, false, false);

Job demoJob("Birou", "TechCorp", "programator", 8000.0);

beneficiar demoBen("Maria Popescu", 30, 50);
